...
function buildConnectionString() {
  ...
  if (host && name && user && password) {
    const useSsl = process.env.DB_SSL !== "false" && host !== "localhost" && host !== "127.0.0.1";
...
async function handler(req, res) {
  try {
    ...
    if (req.method === "POST") {
      const { name } = req.body;
      if (!name?.trim()) return res.status(400).json({ error: "Thi\u1EBFu t\xEAn h\xE3ng" });
    ...
    return res.status(500).json({ error: "L\u1ED7i khi x\u1EED l\xFD h\xE3ng camera" });
  }
}
export {
  handler as default
